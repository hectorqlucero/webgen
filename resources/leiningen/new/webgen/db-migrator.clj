...
(defn copy-all-data
  ...
  (let [target-type (keyword (or (first args) "mysql"))
    ...
    (println (str "Source: SQLite (db/{{sanitized}}.db)"))
    ...
    (try
      ...
      (catch Exception e
        ...
        (println "\nMake sure:")
        (println "1. SQLite database exists at db/{{sanitized}}.db")
        (println "2. Target database server is running")
        (println "3. Target database credentials are correct in {{sanitized}}.db.migrator/db-configs")
