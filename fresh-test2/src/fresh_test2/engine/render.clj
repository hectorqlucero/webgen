(ns fresh_test2.engine.render
  ...
  (:require
   ...
   [fresh_test2.models.grid :as grid]
   [fresh_test2.models.form :as form]
   [fresh_test2.models.crud]
            ...
            filename (if (and (string? field-value) (re-find #"^<img" field-value))
                       ...
                       (when-let [match (re-find #"src='([^']+)'" field-value)]
                         (-> (second match)
                             ...
                             first
                             (clojure.string/replace (:path fresh_test2.models.crud/config) "")))
                       field-value)]
        [:div.mb-3
         ...
         (when (and filename (not (map? filename)) (not (empty? filename)))
           [:div.mb-2
            [:img {:src (str (:path fresh_test2.models.crud/config) filename "?" (random-uuid))
                   :alt filename
